...
const selectReactSettings = createSelector(
  [
    (state: RootState) => state.user.changes?.company_user?.react_settings,
    (state: RootState) =>
      state.user.user?.company_user?.settings?.react_table_columns,
  ],
  (reactSettings, previousReactTableColumns): ReactSettings => {
    ...
    // The returned object is cached by Reselect and shared across every
    // `useReactSettings()` consumer — do not mutate it in place.
    return merge<ReactSettings, ReactSettings>(base, reactSettings ?? {});
  }
);

export function useReactSettings() {
  return useSelector(selectReactSettings);
}
